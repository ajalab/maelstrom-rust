...
enum Command {
    ...
    Cas { key: u64, from: i64, to: i64 },
}

enum CommandResult {
    Read { value: Option<i64> },
    Write,
    Cas { result: Option<Result<(), ()>> },
}

struct CommandHandlerContext<'a> {
    state: &'a mut State,
    stub: &'a mut Stub,
}

type CommandHandlerCallback =
    Box<dyn FnOnce(CommandResult, CommandHandlerContext<'_>, &CommandHandler) + Send>;

struct CommandHandler {
    callback: Option<CommandHandlerCallback>,
    src: String,
    dest: String,
    msg_id: u64,
}
...
impl CommandHandler {
    fn from_message(msg: Message<MessageBody>) -> (Command, Self) {
        let msg_id = msg.body.msg_id();
        let command = match msg.body {
            MessageBody::Read { key, .. } => Command::Read { key },
            MessageBody::Write { key, value, .. } => Command::Write { key, value },
            MessageBody::Cas { key, from, to, .. } => Command::Cas { key, from, to },
            _ => unreachable!("only lin-kv commands have handlers"),
        };
        let callback = Box::new(
            |result: CommandResult, ctx: CommandHandlerContext, handler: &CommandHandler| {
                let msg_id = ctx.state.acquire_message_id();
                let body = match result {
                    CommandResult::Read { value: Some(value) } => MessageBody::ReadOk {
                        msg_id,
                        in_reply_to: handler.msg_id,
                        value,
                    },
                    CommandResult::Read { value: None } => MessageBody::Error {
                        msg_id,
                        in_reply_to: handler.msg_id,
                        code: ErrorCode::KeyDoesNotExist,
                        text: "key does not exist".to_string(),
                    },
                    CommandResult::Write => MessageBody::WriteOk {
                        msg_id,
                        in_reply_to: handler.msg_id,
                    },
                    CommandResult::Cas {
                    ...
                    CommandResult::Cas { result: None } => MessageBody::Error {
                        msg_id,
                        in_reply_to: handler.msg_id,
                        code: ErrorCode::KeyDoesNotExist,
                        text: "key does not exist".to_string(),
                ...
                ctx.stub
                    .send_message(&Message {
                        src: handler.dest.clone(),
                        dest: handler.src.clone(),
                        body,
                    })
                    .expect("command response should be writable");
            },
        );
    ...
    fn handle(mut self, result: CommandResult, ctx: CommandHandlerContext<'_>) {
        self.callback.take().expect("command handlers run once")(result, ctx, &self);
    }
}
...
struct State {
    ...
    // Local-only callbacks for commands accepted by this leader. Replicated log
    // entries carry just the serializable command.
    command_handlers: HashMap<u64, CommandHandler>,
}
...
impl Node {
    ...
    fn handle_command(&mut self, msg: Message<MessageBody>) -> Result<()> {
        ...
        let (command, handler) = CommandHandler::from_message(msg);
        ...
        state.command_handlers.insert(index, handler);
        ...
        thread::spawn(move || {
            ...
            loop {
                ...
                while state.last_applied < state.commit_index {
                    ...
                    let command = state.log.get(index).unwrap().command.clone();
                    let command_result = Self::apply_command(&mut state.map, command);

                    if state.role == Role::Leader
                        && let Some(handler) = state.command_handlers.remove(&index)
                    {
                        handler.handle(
                            command_result,
    ...
    fn apply_command(map: &mut HashMap<u64, i64>, command: Command) -> CommandResult {
        match command {
            Command::Read { key } => {
                let value = map.get(&key).copied();
                CommandResult::Read { value }
            }
